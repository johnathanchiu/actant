...
Revision ID: 0001_actant_schema
Revises:

The five tables Actant's SQLAlchemy runtime stores read and write: threads, runs, messages
and their ordered parts, and tool calls. Autogenerated from ``ACTANT_RUNTIME_METADATA``.

This is a branch, labelled ``actant``. An application embeds it through
``version_locations`` and keeps its own revisions on its own branch.

It replaces revisions ``0001_actant_runtime`` to ``0003_typed_blocks`` (0.20.x) with one
...
schema: point its ``actant`` row at this revision instead of running it::
    ...
    UPDATE alembic_version SET version_num = '0001_actant_schema'
...
revision: str = "0001_actant_schema"
