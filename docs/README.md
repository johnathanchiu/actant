...
1. [Why Actant?](why-actant.md) — the problem Actant removes, its runtime
   guarantees, and how it differs from adjacent agent frameworks.
2. [Core concepts](concepts.md) — the vocabulary and ownership boundaries.
3. [Runtime architecture](architecture.md) — the exact workflow algorithm,
   activity contracts, state ownership, and recommended code-reading order.
4. [Runtime guide](actant-runtime-guide.md) — connect a client and run a worker.
5. [Tools guide](tools-guide.md) — tool schemas, execution, and admission.
6. [Pauses and deferred work](pauses-and-resume.md) — approval and durable wait
   semantics.
7. [Subagents](subagents.md) — synchronous and durable delegation.
8. [Coordinator guide](coordinator-guide.md) — application policy for
   multi-agent products.
9. [Release checklist](releasing.md) — validate, version, and publish the wheel
