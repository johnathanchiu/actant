...
class _BlockingLLM:
    """A child that will not answer until the test lets it."""

    model_id = "fake-blocking"
    ...
    async def complete(
        ...
        del system, messages, tools, listener
        self.entered.set()
...
@pytest.mark.asyncio
async def test_cancelling_the_parent_cancels_the_child() -> None:
    """A cancelled parent does not leave its subagent running.

    Cancel arrives as a signal, which only sets a flag -- so an await on
    the child has to watch for it. Without that the parent sits waiting on
    work nobody wants any more, and pays for all of it.
    """
    tool_call = _tool_call("delegate")
    ...
    async with await WorkflowEnvironment.start_local() as env:
        async with Worker(
            ...
            # The child is in flight and will not answer on its own.
            await _wait_for(lambda: blocking.entered.is_set())

            await handle.signal("cancel")

            # The child must stop on its own, without the test ever
            # releasing it. A cancel that did not reach the child would
            # leave this RUNNING until the blocking call returned.
            child_handle = env.client.get_workflow_handle(f"child-of-{tool_call.id}")

            async def child_stopped() -> bool:
                described = await child_handle.describe()
                return described.status is not None and described.status.name != "RUNNING"

            await _wait_for(child_stopped, timeout=20.0)
            assert not blocking.released.is_set(), (
                "the child stopped because it was cancelled, not because it answered"
            )
